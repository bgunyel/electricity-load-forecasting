...
from config import settings, pjm
...
def read_pjm_data(start_year: int, end_year: int, aggregation_level: str) -> pl.DataFrame:
    ...
    if aggregation_level not in pjm.AGGREGATION_MAP.keys():
        raise Exception(f'Aggregation level should be one of {pjm.AGGREGATION_MAP.keys()}')
    ...
    out_df = out_df.with_columns(
        pl.col(pjm.DATE_TIME_UTC).str.to_datetime(format="%m/%d/%Y %I:%M:%S %p"),
        pl.col(pjm.DATE_TIME_EPT).str.to_datetime(format="%m/%d/%Y %I:%M:%S %p"),
    )

    # Local Date Time is assumed to be UTC-5 throughout the year
    # All the analyses will be carried with UTC-5 instead of UTC
    out_df = out_df.with_columns(
        pl.col(pjm.DATE_TIME_UTC).dt.offset_by("-5h").alias(pjm.LOCAL_DATE_TIME)
    )

    out_df = out_df.group_by(
        by=[pjm.LOCAL_DATE_TIME, aggregation_level],
        maintain_order=True
    ).agg(pl.sum(pjm.LOAD))

    out_df = out_df.pivot(
        index=pjm.LOCAL_DATE_TIME,
...
def read_ghcnd_stations(country: str) -> pd.DataFrame:
    ...
    df = pd.DataFrame(q, columns=[pjm.ID, pjm.STATE, pjm.LAT, pjm.LON, pjm.ELEV])
